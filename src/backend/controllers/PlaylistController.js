...
export const getPlaylistsByUser = async (req, res) => {
  try {
    ...
    const playlistsWithUrl = playlists.map(pl => ({
      ...pl, cover_url: pl.cover_url && !pl.cover_url.startsWith("/uploads") ? `/uploads/${pl.cover_url}` : pl.cover_url
    }));
