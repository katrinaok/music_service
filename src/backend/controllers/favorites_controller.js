import Favourite from "../models/favorites_model.js";
...
export const getFavoriteById = async (req, res) => {
  try {
    ...
    if (!favourite) return res.status(404).json({ message: "Улюблене не знайдене" });
    res.json(favourite);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
export const getPlaylistTracks = async (req, res) => {
  ...
  try {
    ...
    res.json({ tracks: tracksWithFavorites });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Не вдалося завантажити треки плейлісту" });
  }
};
...
export const updateFavorite = async (req, res) => {
  try {
    ...
    res.json({ message: "Улюблене оновлено" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
export const deleteFavorite = async (req, res) => {
  try {
    ...
    if (!user_id || !track_id) return res.status(400).json({ error: "Потрібні user_id і track_id" });
    
    await Favourite.deleteByUserAndTrack(user_id, track_id);
    res.json({ message: "Улюблене видалено" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};