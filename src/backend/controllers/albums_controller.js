...
export const getAlbumById = async (req, res) => {
  try {
    ...
    if (!album) return res.status(404).json({ message: "Альбом не знайдений" });
    res.json(album);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
export const createAlbum = async (req, res) => {
  try {
    ...
    const newAlbum = { title, artist_id, release_year: year, cover_url };
    
    const result = await Album.create(newAlbum);
    res.status(201).json({ id: result.id, ...newAlbum });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
export const updateAlbum = async (req, res) => {
  try {
    ...
    await Album.update(req.params.id, { ...req.body, cover_image });
    res.json({ message: "Альбом оновлений" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
export const deleteAlbum = async (req, res) => {
  try {
    ...
    res.json({ message: "Альбом видалений" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};