...
export const getArtistById = async (req, res) => {
  try {
    ...
    if (!artist) return res.status(404).json({
      message: "Виконавець не знайдений"
    });
    res.json(artist);
  } catch (err) {
    res.status(500).json({
...
export const createArtist = async (req, res) => {
  try {
    ...
    if (!name) return res.status(400).json({
      error: "Ім'я виконавця обов'язкове"
    });
    ...
    res.status(201).json({
      id: result.id, name, bio, image_url
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({
...
export const updateArtist = async (req, res) => {
  try {
    await Artist.update(req.params.id, req.body); res.json({
      message: "Виконавець оновлений"
    });
  } catch (err) {
    res.status(500).json({
      error: err.message
