...
export const getPlaylistTracks = async (req, res) => {
  try {
    ...
    if (isNaN(playlist_id)) return res.status(400).json({
      error: "Некоректний playlist_id"
    });
    ...
    console.error(err);
    res.status(500).json({
      error: "Не вдалося завантажити треки плейліста"
    });
  }
};
...
export const addTrackToPlaylist = async (req,res)=>{
  try {
    ...
    if(isNaN(playlist_id)||isNaN(track_id)) return res.status(400).json({
      error:"Некоректний playlist_id або track_id"
    });
...
export const deleteTrackFromPlaylist = async (req, res) => {
  try {
    ...
    res.json({
      message: "Трек видалено з плейліста"
    });
  } catch (err) {
    res.status(500).json({
      error: err.message
