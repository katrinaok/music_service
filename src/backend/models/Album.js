...
const Album = {
  ...
  create: async (album) => {
    ...
    const [result] = await db.query( "INSERT INTO albums (title, artist_id, release_year, cover_url) VALUES (?, ?, ?, ?)",
      [title, artist_id, release_year, cover_url]
    );
  ...
  update: async (id, album) => {
    ...
    await db.query( "UPDATE albums SET title=?, artist_id=?, release_year=?, cover_url=? WHERE id=?",
