...
const Track = {
  ...
  create: async (track) => {
    const { title, artist_id, album_id, genre_id, file_path } = track;
    ...
    const [result] = await db.query(
      "INSERT INTO tracks (title, artist_id, album_id, genre_id, file_path) VALUES (?, ?, ?, ?, ?)",
  ...
  update: async (id, track) => {
    const { title, artist_id, album_id, genre_id, duration } = track;
    await db.query(
      "UPDATE tracks SET title = ?, artist_id = ?, album_id = ?, genre_id = ?, duration = ? WHERE id = ?",
