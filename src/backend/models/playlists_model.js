...
const Playlist = {
  ...
  async getByUserId(userId) {
    const [rows] = await db.query(
      "SELECT id, name, description, cover_url, user_id FROM playlists WHERE user_id = ? ORDER BY id ASC",
      [userId]
    );
    return rows;
  },
  ...
  async create(playlist) {
    const { name, user_id, description, cover_url } = playlist;
    ...
    const [result] = await db.query(
      "INSERT INTO playlists (name, user_id, description, cover_url) VALUES (?, ?, ?, ?)",
      [name, user_id, description, cover_url]
    );

    return { id: result.insertId, name, user_id, description, cover_url };
  },

  async update(id, playlist) {
    const { name, user_id, description, cover_url } = playlist;
    await db.query(
      "UPDATE playlists SET name = ?, user_id = ?, description = ?, cover_url = ? WHERE id = ?",
